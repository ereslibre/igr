...
QList<PV3f> Frenet::marco(const QList<PV3f> &listaPuntos, GLdouble t, GLdouble b)
{
    ...
    return multMatriz(listaPuntos, vn, vt, vb, t, b);
}
...
QList<PV3f> Frenet::multMatriz(const QList<PV3f> &listaPuntos, const PV3f &n, const PV3f &b, const PV3f &t, GLdouble s, GLdouble bb)
{
    QList<PV3f> res;
    foreach (const PV3f &p, listaPuntos) {
        ...
        res << PV3f(cos(s), sin(s), s * bb) + PV3f(n.dot(p), b.dot(p), t.dot(p), PV3f::Vector);
    }
    return res;
}
